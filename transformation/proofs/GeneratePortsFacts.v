...
Section GenActionPorts.
  ...
  Proof. intros * EQ; pattern s, s'; solve_sinv_pattern.
         minv EQ3; cbn.
         intros [ id__p [ g__p [ i__p [ o__p [ InA_p2pdi InCs_ ] ] ] ] ];
...
Section GenFunPorts.
  ...
  Lemma gen_fports_inv_p2pdi :
    forall {sitpn s v s'},
      generate_fun_ports_and_ps sitpn s = OK v s' ->
      p2pdi (γ s) = p2pdi (γ s').
  Proof. intros * EQ; pattern s, s'; solve_sinv_pattern. Qed.

  Lemma gen_fports_inv_t2tdi :
    forall {sitpn s v s'},
      generate_fun_ports_and_ps sitpn s = OK v s' ->
      t2tdi (γ s) = t2tdi (γ s').
  Proof. intros * EQ; pattern s, s'; solve_sinv_pattern. Qed.
...
Section GenCondPorts.
  ...
  Lemma gen_cports_inv_p2pdi :
    forall {sitpn s v s'},
      generate_and_connect_cond_ports sitpn s = OK v s' ->
      p2pdi (γ s) = p2pdi (γ s').
  Proof. intros * EQ; pattern s, s'; solve_sinv_pattern. Qed.

  Lemma gen_cports_inv_t2tdi :
    forall {sitpn s v s'},
      generate_and_connect_cond_ports sitpn s = OK v s' ->
      t2tdi (γ s) = t2tdi (γ s').
  Proof. intros * EQ; pattern s, s'; solve_sinv_pattern. Qed.
  ...
  Proof. intros * EQ; pattern s, s'; solve_sinv_pattern.
         minv EQ13.
         intros [ id__p [ g__p [ i__p [ o__p [ InA_p2pdi InCss7 ] ] ] ] ].
         exists id__p.
...
Lemma gen_ports_inv_p2pdi :
  forall {sitpn s v s'},
    @generate_ports sitpn s = OK v s' ->
    p2pdi (γ s) = p2pdi (γ s').
Proof. intros * EQ; pattern s, s'; solve_sinv_pattern. Qed.

Lemma gen_ports_inv_t2tdi :
  forall {sitpn s v s'},
    @generate_ports sitpn s = OK v s' ->
    t2tdi (γ s) = t2tdi (γ s').
Proof. intros * EQ; pattern s, s'; solve_sinv_pattern. Qed.
...
Lemma gen_ports_pdi_ex :
  forall (sitpn : Sitpn) (s : Sitpn2HVhdlState sitpn) v s' p,
    ...
    (exists id__p g__p i__p o__p,
        InA Pkeq (p, id__p) (p2pdi (γ s'))
        /\ InCs (cs_comp id__p Petri.place_id g__p i__p o__p) (beh s')).
Proof. intros * EQ pdi_ex; monadInv EQ.
       eapply gen_cports_pdi_ex; eauto;
         eapply gen_fports_pdi_ex; eauto;
         eapply gen_aports_pdi_ex; eauto.
Qed.
