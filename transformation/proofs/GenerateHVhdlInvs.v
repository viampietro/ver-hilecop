...
(*       InCs (cs_comp id__p Petri.place_id gm ipm opm) (beh s). *)
(*   Proof. *)
(*     intros until opm; intros e nPeq InA_p2pdi InCs_beh; minv e; shelf_state EQ5. *)
(*     (* Stores the first subgoal (need it in the second subgoal). *) *)
(*     assert (InA_p2pdi0 : InA Pkeq (x, id__p) (p2pdi (γ s0))). *)
(*     { eapply @InA_setv_inv_2 with (eqk_dec := Peqdec) (v := nextid s0); eauto. *)
(*       change (setv Peqdec y (nextid s0) (p2pdi (γ s0))) with (p2pdi (γ s)). *)
(*       assert (e : p2pdi (γ s4) = (p2pdi (γ s))). *)
...
(*   Lemma gen_p_comp_inst_inv_p2pdi : *)
(*     forall {sitpn x y s v s'}, *)
(*       generate_place_comp_inst sitpn y s = OK v s' -> *)
(*       ~Peq y x -> *)
(*       ~InA Peq x (fs (p2pdi (γ s))) -> *)
(*       ~InA Peq x (fs (p2pdi (γ s'))). *)
(*   Proof. *)
...
(*     intros nex_InA; destruct 1 as (p1, (id__p1, (InA_p2pdi0, GE_nxtid0))). *)
...
(*     change (p2pdi (γ s)) *)
(*       with (setv Peqdec p (nextid s0) (p2pdi (γ s0))) in InA_p2pdi0. *)
...
(*     (* CASE [InA Pkeq (p1, id__p1) (p2pdi (γ s0))] *) *)
...
(*   Lemma iter_gen_pcomp_inst_inv_nodup_p2pdi : *)
...
(*       intros s v s' e; monadInv e; auto; intros *; intros nInA_p2pdi nInA_pls. *)
(*     eapply gen_p_comp_inst_inv_p2pdi; eauto. *)
(*     intros Peq_; apply nInA_pls; symmetry in Peq_; auto. *)
(*   Qed. *)

(*   Lemma iter_gen_pcomp_inst_nodup_p2pdi : *)
(*     forall {sitpn pls s v s'}, *)
(*       iter (generate_place_comp_inst sitpn) pls s = OK v s' -> *)
(*       NoDupA Peq pls -> *)
...
(*     eapply gen_p_comp_inst_nodup_p2pdi; eauto; *)
(*       lazymatch goal with *)
(*       | [ Hnd: NoDupA _ (?a :: ?tl), Hnin: forall _, _ -> ~_ |- _ ] => *)
(*         (eapply iter_gen_pcomp_inst_inv_nodup_p2pdi; eauto; inversion Hnd; assumption) *)
...
(*   Lemma gen_t_comp_inst_inv_p2pdi : *)
...
(*   Lemma gen_t_comp_inst_nodup_t2tdi : *)
(*     forall {sitpn t s v s'}, *)
(*       generate_trans_comp_inst sitpn t s = OK v s' -> *)
(*       ~InA Teq t (fs (t2tdi (γ s))) -> *)
(*       NoDupA Teq (fs (t2tdi (γ s))) -> *)
(*       NoDupA Teq (fs (t2tdi (γ s'))). *)
(*   Proof. *)
...
(*   Lemma gen_t_comp_inst_inv_t2tdi : *)
(*     forall {sitpn x y s v s'}, *)
(*       generate_trans_comp_inst sitpn y s = OK v s' -> *)
(*       ~Teq y x -> *)
(*       ~InA Teq x (fs (t2tdi (γ s))) -> *)
(*       ~InA Teq x (fs (t2tdi (γ s'))). *)
(*   Proof. *)
...
(*   Lemma iter_gen_tcomp_inst_inv_nodup_t2tdi : *)
(*     forall {sitpn trs} {s v s'}, *)
(*       iter (generate_trans_comp_inst sitpn) trs s = OK v s' -> *)
(*       forall t, *)
(*         ~InA Teq t (fs (t2tdi (γ s))) -> *)
...
(*       intros s v s' e; monadInv e; auto; intros *; intros nInA_t2tdi nInA_trs. *)
(*     eapply gen_t_comp_inst_inv_t2tdi; eauto. *)
(*     intros Teq_; apply nInA_trs; symmetry in Teq_; auto. *)
(*   Qed. *)
  
(*   Lemma iter_gen_tcomp_inst_nodup_t2tdi : *)
(*     forall {sitpn trs s v s'}, *)
(*       iter (generate_trans_comp_inst sitpn) trs s = OK v s' -> *)
(*       NoDupA Teq trs -> *)
(*       (forall t, InA Teq t trs -> ~InA Teq t (fs (t2tdi (γ s)))) -> *)
(*       NoDupA Teq (fs (t2tdi (γ s))) -> *)
(*       NoDupA Teq (fs (t2tdi (γ s'))). *)
(*   Proof. *)
(*     intros until trs; *)
...
(*     eapply gen_t_comp_inst_nodup_t2tdi; eauto; *)
(*       lazymatch goal with *)
(*       | [ Hnd: NoDupA _ (?a :: ?tl), Hnin: forall _, _ -> ~_ |- _ ] => *)
(*         (eapply iter_gen_tcomp_inst_inv_nodup_t2tdi; eauto; inversion Hnd; assumption) *)
(*         || (eapply IHm; eauto; inversion Hnd; assumption) *)
(*       end. *)
(*   Qed. *)
...
(*     split; *)
(*       [rewrite <- (iter_gen_tcomp_inst_inv_p2pdi EQ0); assumption | *)
(*        eapply iter_gen_tcomp_inst_inv_in_beh; eauto]. *)
(*   Qed. *)

(*   Lemma gen_tcomp_insts_inv_p2pdi : *)
(*     forall {sitpn s v s'}, *)
(*       generate_trans_comp_insts sitpn s = OK v s' -> *)
(*       p2pdi (γ s) = p2pdi (γ s'). *)
(*   Proof. *)
(*     intros *; intros e; minv e. *)
(*     eapply iter_gen_tcomp_inst_inv_p2pdi; eauto. *)
(*   Qed. *)
...
(*   Lemma gen_tcomp_insts_nodup_t2tdi : *)
(*     forall {sitpn : Sitpn} {s : Sitpn2HVhdlState sitpn} {v s'}, *)
(*       generate_trans_comp_insts sitpn s = OK v s' -> *)
(*       Sig_in_List (lofTs s) -> *)
(*       (forall t, ~InA Teq t (fs (t2tdi (γ s)))) -> *)
(*       NoDupA Teq (fs (t2tdi (γ s))) -> *)
(*       NoDupA Teq (fs (t2tdi (γ s'))). *)
(*   Proof. *)
(*     intros *; intros e; minv e; intros SIL nInA. *)
(*     eapply iter_gen_tcomp_inst_nodup_t2tdi; eauto. *)
(*     exact (proj2 SIL). *)
(*   Qed. *)
...
(*   Lemma gen_comp_insts_nodup_p2pdi : *)
(*     forall {sitpn : Sitpn} {s : Sitpn2HVhdlState sitpn} {v s'}, *)
(*       generate_comp_insts sitpn s = OK v s' -> *)
(*       Sig_in_List (lofPs s) -> *)
(*       (forall p, ~InA Peq p (fs (p2pdi (γ s)))) -> *)
(*       NoDupA Peq (fs (p2pdi (γ s))) -> *)
(*       NoDupA Peq (fs (p2pdi (γ s'))). *)
(*   Proof. *)
(*     intros until s'; intros e; monadInv e; intros. *)
(*     minv EQ0; rewrite <- (iter_gen_tcomp_inst_inv_p2pdi EQ2). *)
(*     minv EQ; eapply iter_gen_pcomp_inst_nodup_p2pdi; eauto. *)
(*     apply (proj2 H). *)
(*   Qed. *)

(*   Lemma gen_pcomp_insts_inv_t2tdi : *)
(*     forall {sitpn s v s'}, *)
(*       generate_place_comp_insts sitpn s = OK v s' -> *)
(*       t2tdi (γ s) = t2tdi (γ s'). *)
(*   Admitted. *)
  
(*   Lemma gen_comp_insts_nodup_t2tdi : *)
(*     forall {sitpn : Sitpn} {s : Sitpn2HVhdlState sitpn} {v s'}, *)
(*       generate_comp_insts sitpn s = OK v s' -> *)
(*       Sig_in_List (lofTs s) -> *)
(*       (forall t, ~InA Teq t (fs (t2tdi (γ s)))) -> *)
...
(*     erewrite @gen_pcomp_insts_inv_t2tdi with (s' := s0); eauto. *)
(*     erewrite @gen_pcomp_insts_inv_lofTs with (s' := s0); eauto. *)
(*     eapply gen_tcomp_insts_nodup_t2tdi; eauto. *)
(*   Qed. *)
