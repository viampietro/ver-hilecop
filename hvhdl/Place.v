...
Definition input_arcs_number  : ident := rst + 1.
...
(* Process "input_tokens_sum" sensitivity list. *)
Definition in_toks_sum_sl : IdSet := {input_transitions_fired; input_arcs_weights}.

(* Process "input_tokens_sum" declarative part. *)

Definition v_internal_input_token_sum : ident := local_var.
Definition in_toks_sum_vars : list vdecl := [vdecl_ v_internal_input_token_sum local_weight_t].

(* Process "input_tokens_sum" sequential statement body. *)

Definition in_toks_sum_body : ss :=
  $v_internal_input_token_sum @:= (e_nat 0);;

  For i From (e_nat 0) To (#input_arcs_number @- (e_nat 1)) Do
...
Definition in_toks_sum_ps := cs_ps
                               input_tokens_sum
                               in_toks_sum_sl in_toks_sum_vars
                               in_toks_sum_body.
