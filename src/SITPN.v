 ...
 *            if it exists.
 *)
Definition check_condition (conditions : conditions_type) (t : trans_type) : bool :=
...
| check_condition_none : 
    (conditions t) = None -> check_condition_spec conditions t
| check_condition_some_if : 
    (conditions t) = Some true -> check_condition_spec conditions t.
...
Theorem check_condition_correct :
  forall (conditions : conditions_type)
         (t : trans_type),
  (check_condition conditions t) = true -> (check_condition_spec conditions t).
Proof.
  intros conditions t.
...
(* 
 * Function : Same behavior as stpn_class_fire_pre_aux, except
 *            that the condition associated to each transition
 *            is checked and the if clause.
 *)
...
(*** Formal specification : sitpn_class_fire_pre_aux ***)
...
| class_nil :
    forall (m_decreased : marking_type)
           ...
           (chronos : trans_type -> option chrono_type),
    sitpn_class_fire_pre_aux_spec whole_class places pre test inhib m_steady conditions 
                                  m_decreased chronos [] subclass_fired_pre subclass_fired_pre m_decreased chronos
| class_cons_if :
    forall (t : trans_type)
     ...
     new_chronos = (reset_all_chronos0 (reset_chrono0 chronos t)
                                       (list_disabled whole_class places pre test inhib m_steady m_decreasing_low)) ->
     sitpn_class_fire_pre_aux_spec whole_class places pre test inhib m_steady conditions
                                   m_decreasing_low new_chronos tail (subclass_fired_pre ++ [t]) sub m chronos_final ->
     sitpn_class_fire_pre_aux_spec whole_class places pre test inhib m_steady conditions
                                   m_decreasing_high chronos (t::tail) subclass_fired_pre sub m chronos_final
...
(*** Two lemmas to resolve correctness proof. ***)
...
(*** Correctness proof : sitpn_class_fire_pre_aux ***)
...
(*** Completeness proof : sitpn_class_fire_pre_aux ***)
Theorem stpn_class_fire_pre_aux_complete :
  forall (whole_class : list trans_type)
         (places : list place_type)
         (pre  test  inhib : weight_type)
         (m_steady   m_decreasing     m_final : marking_type)
         (class_transs  subclass_fired_pre  sub_final : list trans_type)
...
Proof.
  ...
  - simpl. reflexivity.
  - intros. simpl.
    assert (H0' : check_all_edges places (pre t) (test t) (inhib t) m_steady m_decreasing_high
                  && check_chrono (chronos0 t)
                  && (check_condition conditions t) = true).
    { apply andb3_true_iff. assumption. }  
    rewrite H0'. rewrite <- H1. rewrite <- H2. rewrite H4. reflexivity.
  - intros. simpl.
...
(*
 * Function : Wrapper function around sitpn_class_fire_pre_aux. 
 *)
Definition sitpn_class_fire_pre
           ...
           (conditions : conditions_type) :
  list trans_type * marking_type * (trans_type -> option chrono_type) :=
  sitpn_class_fire_pre_aux class_transs places pre test inhib m_steady m_decreasing
                           chronos class_transs [] conditions.

(*** Formal specification : sitpn_class_fire_pre ***)
...
(*** Correctness proof : sitpn_class_fire_pre ***)
...
(*** Completeness proof : sitpn_class_fire_pre ***)
...
Fixpoint sitpn_fire_pre_aux
  ...
  | class :: Ltail =>
    let '(sub_l, new_m, new_chronos) :=
        sitpn_class_fire_pre places pre test inhib m_steady m_decreasing chronos class conditions in
    sitpn_fire_pre_aux places pre test inhib m_steady new_m new_chronos Ltail (sub_l :: classes_half_fired) conditions
  end.

(*** Formal secification : sitpn_fire_pre_aux ***)
...
Proof.
  ...
  - intro H.
    apply classes_cons
      with (class_fired_pre := fst_tuplet (sitpn_class_fire_pre
                                             places pre test inhib
                                             m_steady
                                             m_decreasing chronos
                                             class  conditions))
           (m_decreased := snd_tuplet (sitpn_class_fire_pre
                                         places pre test inhib
                                         m_steady
                                         m_decreasing chronos
                                         class   conditions ))
           (chronos_final := trd_tuplet (sitpn_class_fire_pre
                                      places pre test inhib
                                      m_steady
                                      m_decreasing chronos
                                      class  conditions)).
...
(*
 * Function : Wrapper around the sitpn_fire_pre_aux function.
 *            Initializes 
 *)
Definition sitpn_fire_pre
           ...
           (conditions : conditions_type ) :
  list (list trans_type) * marking_type * (trans_type -> option chrono_type) :=
  sitpn_fire_pre_aux places pre test inhib m_steady m_steady chronos classes_transs [] conditions.
...
| sitpn_fire_pre_mk :
    forall (classes_fired_pre : list (list trans_type))
      ...
      sitpn_fire_pre_spec places pre test inhib m_steady chronos classes_transs conditions
                          classes_fired_pre m_inter chronos_next.
...
Definition sitpn_debug1
  ...
  let '(sub_Lol, m_inter, new_chronos ) :=
      (sitpn_fire_pre places pre test inhib marking chronos classes_transs conditions)
...
Definition sitpn_debug2 (sitpn : SITPN) :
  ...
  match sitpn with
  | mk_SITPN (mk_STPN (mk_SPN places transs pre test inhib post marking (mk_prior Lol)) chronos) scenario =>
    match scenario with
    | C :: tail => sitpn_debug1 places transs pre test inhib marking chronos Lol C
    | [] => ([], [], [])
    end
  end.

(* Returns [transitions fired + final marking + chronos] *)
Definition sitpn_fire  
  ...
  (trans_type -> option chrono_type)  :=
  let '(sub_Lol, m_inter, new_chronos) :=
      sitpn_fire_pre places pre test inhib m_steady chronos classes_transs conditions in
  (sub_Lol, fire_post places post m_inter sub_Lol, new_chronos).
...
| sitpn_fire_mk :
    forall (sub_Lol : list (list trans_type))
      ...
      (sub_Lol, m_inter, new_chronos) = sitpn_fire_pre places pre test inhib m_steady
                                                       chronos classes_transs conditions ->
      m = fire_post places post m_inter sub_Lol ->
      sitpn_fire_spec places pre test inhib post m_steady chronos classes_transs conditions sub_Lol m new_chronos.
...
(*
 * The marking and the "intervals" (their counter) are evolving !!  
 ...
 *            transitions - with right chrono, and condition values - in sitpn 
 *            have been fired, and returns the list of lists containing these transitions.
 *            
 *)
Definition sitpn_cycle (sitpn : SITPN) : (list (list trans_type)) * SITPN :=
  match sitpn with
  | mk_SITPN (mk_STPN (mk_SPN places transs pre post test inhib marking (mk_prior Lol)) chronos) scenario =>
    match scenario with
    | [] => ([], sitpn) (* dangerous ? freeze! or continue with no more constraints ? *)
    | C :: tail =>
      let chronos_incr :=
...
Inductive sitpn_cycle_spec (sitpn : SITPN) : list (list trans_type) -> SITPN -> Prop :=
| sitpn_fired_mk_nil :
    forall (Lol: list (list trans_type))
           ...
           (chronos : trans_type -> option chrono_type)
           (scenario : scenar_type),
    sitpn = mk_SITPN (mk_STPN (mk_SPN places transs pre post test inhib m (mk_prior Lol)) chronos) scenario ->
    scenario = [] -> sitpn_cycle_spec sitpn [] sitpn
| sitpn_fired_mk :
    ...
    sitpn = mk_SITPN stpn scenario ->
    scenario = C :: Tail ->
    enabled = list_sensitized_spn spn -> chronos_incr = increment_all_chronos chronos enabled -> 
    (sub_Lol, next_m, next_chronos ) = (sitpn_fire places pre test inhib post m chronos_incr Lol C) ->
    next_sitpn = mk_SITPN (mk_STPN (mk_SPN places transs pre post test inhib next_m (mk_prior Lol)) next_chronos) Tail -> 
    sitpn_cycle_spec sitpn sub_Lol next_sitpn.
...
Fixpoint sitpn_animate (sitpn : SITPN) (n : nat) :
  list (list (list trans_type) *
        list (place_type * nat) *
        (list (trans_type * option (nat * nat * nat)))) :=
  match n with
  | O => [ ([], [], []) ]
  | S n' =>  let (fired, next_sitpn) := (sitpn_cycle sitpn) in
             (fired,
              (marking2list
              ...
              (intervals2list
               ...
               ::
               (sitpn_animate next_sitpn n')
  end.

(*** Formal specification : sitpn_animate ***)
Inductive sitpn_animate_spec (sitpn : SITPN) :
  nat ->
  list
    (list (list trans_type)  *
     list (place_type * nat) *
     (list (trans_type * option (nat * nat * nat)))) ->
  Prop :=
| animate_sitpn_O : sitpn_animate_spec sitpn O [ ([], [], []) ]
| animate_sitpn_S :
