...
Record Sitpn  :=
  BuildSitpn {
      ...
      pr : T -> T -> bool;
      pr_irrefl : forall t, pr t t = false;
      pr_antisym : forall t t', pr t t' = true -> pr t' t = false;
      pr_trans : forall t t' t'', pr t t' = true -> pr t' t'' = true -> pr t t'' = true;
...
Definition AllConditionsTrue (sitpn : Sitpn) (s : SitpnState sitpn) (t : (T sitpn)) :=
  forall c, has_C t c = true -> cond s c = true.
...
Definition Firable (sitpn : Sitpn) (s : SitpnState sitpn) (t : T sitpn) :=
  Sens (M s) t /\ HasReachedTimeWindow s t /\ AllConditionsTrue s t.
