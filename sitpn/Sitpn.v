...
Record Sitpn  :=
  BuildSitpn {
      ...
      (** A list of nat implementing the finite set of places. *)
      places : list nat;
      
      (** A list of nat implementing the finite set of transitions. *)
      transitions : list nat;
      
      (** Alias for the set of elements that belong to the [places]
          list. *)
      In_P := (fun p => In p places);
      P := { p | In_P p };
      
      (** Alias for the set of elements that belong to the
          [transitions] list. *)
      ...
      (** Associates a time interval to transitions of the SITPN. *)
      Is : T -> option TimeInterval;

      (** Lists implementing the finite sets of conditions (C),
          actions (A) and functions (F). *)
      conditions : list nat;
      actions : list nat;
      functions : list nat;

      (** Aliases for the set of elements that belong to the
         [conditions] (resp. [actions] and [functions]) list. *)
...
(** ** Subsets of P and T, and misc. coercion functions *)
